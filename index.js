 ...
 * @returns boolean
 */
async function externalEvaluation(claims,event) {
  //console.log(claims)
  //console.log(event.request.cf.timezone)
  const now = new Date();
  const hourNow = now.toLocaleString("pt-PT", {timeZone: event.request.cf.timezone, hour: 'numeric'})
  const dayNow = now.toLocaleString("en-US", {timeZone: event.request.cf.timezone, weekday: 'short'})

  // External Evaluation will allow access if weekday between 8:00 and 17:59
  if (dayNow != "Sat" && dayNow != "Sun" && hourNow >= 8 && hourNow <= 17){
    console.log("now: "+now+" // hournow: "+hourNow+" // daynow: "+dayNow+" // timezone: "+event.request.cf.timezone+" // result: true");
    return true
  } else {
    console.log("now: "+now+" // hournow: "+hourNow+" // daynow: "+dayNow+" // timezone: "+event.request.cf.timezone+" // result: false");
    return false
  }
}
...
async function handleExternalEvaluationRequest(event) {
  ...
  try {
    ...
    if (claims) {
      ...
      if (await externalEvaluation(claims,event)) {
        result.success = true
      }
    }
